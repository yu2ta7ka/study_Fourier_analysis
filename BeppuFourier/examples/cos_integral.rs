...
fn main() -> ! {
    ...
    writeln!(&mut tx, "calculate cos integral.").unwrap();

    const DIV_NUM: usize = 100;
    let delta: f32 = 2.0 * PI / DIV_NUM as f32;
    let mut cos_array: [f32; DIV_NUM] = [0.0; DIV_NUM];
    let mut sum: [f32; DIV_NUM] = [0.0; DIV_NUM];

    for i in 0..DIV_NUM {
        cos_array[i] = (delta * i as f32).cos();
    }

    let start_count = COUNTER.load(Ordering::Relaxed);
    // integration
    for i in 0..DIV_NUM {
        // increase the integration range
        for j in 0..=i {
